...
export const PageContainerFooter = ({
  ...
  return <Footer className={className} pageContainerType={pageContainerType} />
}
...
const PageContainer = (props: PageContainerProps) => {
  const {
    ...
    children,
    header,
    footer: propFooter = true, // Tetap berikan nilai default true untuk backward compatibility
  } = props

  const { layout } = useThemeStore()
  const { pageContainerReassemble } = useLayout()

  // Gunakan nilai showFooter dari themeConfig jika propFooter adalah true (default)
  // Jika propFooter secara eksplisit diatur ke false, maka footer tidak akan ditampilkan
  const footer = propFooter === true ? layout.showFooter : propFooter
